...
import { GetInfoRequest as ServiceGetInfoRequest } from './classes/service/GetInfoRequest';
import { GetInfoResponse as ServiceGetInfoResponse } from './classes/service/GetInfoResponse';
...
class ServiceClient {
	...
	async getInfo(
		param: ServiceGetInfoRequest,
		headers?: {[key: string]: string},
		options?: {[key: string]: any}
	): Promise<ServiceGetInfoResponse> {
		const resp = await fetch(
			this.baseURL + "/service/info?" + (new URLSearchParams(param.toObject())).toString(),
			{
				method: "GET",
				headers: {
					...this.headers,
		...
		return new ServiceGetInfoResponse(await resp.json());
	}
	async getArticle(
