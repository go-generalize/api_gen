...
func (ctrl *postAController) PostA(
	...
	if ago != "" {
		if err := json.Unmarshal([]byte(ago), option); err != nil {
			log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/a): %+v", err)
	...
	err = fs.WalkDir(mock.MockJSONFS, "json/post_a", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
