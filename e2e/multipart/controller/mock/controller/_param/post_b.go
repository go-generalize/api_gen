...
import (
	...
	mock "github.com/go-gneralize/api_gen/v2/e2e/multipart/controller/mock"
...
// NewPostBController ...
func NewPostBController(cp interface{}) PostBController {
...
// @Accept json
// @Produce json
// @Param body body types.PostBRequest true "request parameter"
// @Param param path string true "Param"
// @Success 200 {object} types.PostBResponse
// @Router /{param}/b [post]
func (ctrl *postBController) PostB(
	...
	if ago != "" {
		if err := json.Unmarshal([]byte(ago), option); err != nil {
			log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/_param/b): %+v", err)
	...
	type Mock struct {
		Meta struct {
			...
			MatchRequest *types.PostBRequest `json:"match_request"`
		} `json:"meta"`
		Payload interface{} `json:"payload"`
	}

	jsons := make(map[string]*Mock)
	err = fs.WalkDir(mock.MockJSONFS, "json/_param/post_b", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
