// Package controller is for GET article
...
func NewGetArticleController(cp interface{}) GetArticleController {
...
func (ctrl *getArticleController) GetArticle(
	...
	if ago != "" {
		if err := json.Unmarshal([]byte(ago), option); err != nil {
			log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/service/article): %+v", err)
	...
	type Mock struct {
		Meta struct {
			...
			MatchRequest *types.GetArticleRequest `json:"match_request"`
		} `json:"meta"`
		Payload interface{} `json:"payload"`
	}

	jsons := make(map[string]*Mock)
	err = fs.WalkDir(mock.MockJSONFS, "json/service/get_article", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
