// Package controller is for GET user_job_get
...
func NewGetUserJobGetController(cp interface{}) GetUserJobGetController {
...
func (ctrl *getUserJobGetController) GetUserJobGet(
	...
	if ago != "" {
		if err := json.Unmarshal([]byte(ago), option); err != nil {
			log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/service/user2/_userID/user_job_get): %+v", err)
	...
	type Mock struct {
		Meta struct {
			...
			MatchRequest *types.GetUserJobGetRequest `json:"match_request"`
		} `json:"meta"`
		Payload interface{} `json:"payload"`
	}

	jsons := make(map[string]*Mock)
	err = fs.WalkDir(mock.MockJsonFS, "json/service/user2/userID/get_user_job_get", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
