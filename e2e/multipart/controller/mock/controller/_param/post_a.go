...
// NewPostAController ...
func NewPostAController(cp interface{}) PostAController {
...
// @Accept json
// @Produce json
// @Param body body types.PostARequest true "request parameter"
// @Param param path string true "Param"
// @Success 200 {object} types.PostAResponse
// @Router /{param}/a [post]
func (ctrl *postAController) PostA(
	...
	if ago != "" {
		if err := json.Unmarshal([]byte(ago), option); err != nil {
			log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/_param/a): %+v", err)
	...
	type Mock struct {
		Meta struct {
			...
			MatchRequest *types.PostARequest `json:"match_request"`
		} `json:"meta"`
		Payload interface{} `json:"payload"`
	}

	jsons := make(map[string]*Mock)
	err = fs.WalkDir(mock.MockJSONFS, "json/_param/post_a", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
